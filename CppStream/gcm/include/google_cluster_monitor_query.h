...
typedef struct cm_two_result_str {
  ...
  cm_two_result_str(long long timestamp, long job_id, float sum_cpu, long count)
      : timestamp(timestamp), job_id(job_id), sum_cpu(sum_cpu), count(count) {}
...
class TotalCpuPerCategoryAggregator {
 public:
  static void sort_final_aggregation(
      const std::vector<cm_one_result>& full_aggregates,
      std::vector<cm_one_result>& final_result);
  static void sort_final_aggregation(
      const std::unordered_map<int, cm_one_result>& buffer,
      std::vector<cm_one_result>& result);
  static void calculate_final_aggregation(
  ...
  static void write_output_to_file(
      const std::vector<cm_one_result>& final_result,
      const std::string& outfile_name);
};

class TotalCpuPerCategoryPartition {
 public:
  static void parse_task_events(const std::string& input_file_name,
                                std::vector<task_event>& buffer);
  static void parse_task_events_from_directory(
      const std::string& input_dir_name, std::vector<task_event>& buffer);
  static void query_simulation(const std::vector<task_event>& buffer,
                               const size_t task_number);
  static std::vector<double> query_partitioner_simulation(
  ...
  static void query_window_simulation(const std::vector<task_event>& buffer,
                                      const size_t task_number);
  static void query_window_partitioner_simulation(
      const std::vector<task_event>& buffer, const std::vector<uint16_t>& tasks,
      std::unique_ptr<Partitioner>& partitioner,
      const std::string& partitioner_name);
};
...
class MeanCpuPerJobIdPartition {
 public:
  static void query_simulation(const std::vector<task_event>& buffer,
                               const size_t task_number);
  static std::vector<double> query_partitioner_simulation(
      bool write, const std::vector<task_event>& buffer,
      const std::vector<uint16_t>& tasks,
      std::unique_ptr<Partitioner>& partitioner,
      const std::string& partitioner_name,
      const std::string& worker_output_file_name);
