...
namespace Experiment
{
	...
	public:
		LogNormalSimulation() {};
		~LogNormalSimulation() {};
		void sort_to_plot(std::string input_file) const
		{
		...
		void simulate(std::vector<uint16_t> tasks, std::string input_file) const
		{
			std::ifstream input(input_file);
			std::string line;
		...
		static void measure_imbalance(const std::vector<unsigned long long>& stream, const std::vector<uint16_t>& tasks, Partitioner& partitioner,
			const std::string partitioner_name)
		{
			...
			key_per_task.clear();
			std::cout << "Time partition using " << partitioner_name << ": " << partition_time.count() << " (msec).\n";
