...
namespace Experiment
{
	namespace DebsChallenge
	{
		...
		public:
			...
			*/
			static void produce_compact_ride_file(const std::string& input_file_name, const std::string& output_file_name,
			...
			static std::vector<Experiment::DebsChallenge::CompactRide> parse_debs_rides(const std::string input_file_name,
				uint32_t cell_side_size, uint32_t grid_side_size_in_cells)
			{
			...
			static void parse_debs_rides_with_to_string(const std::string input_file_name, std::vector<Experiment::DebsChallenge::CompactRide>* buffer)
			{
				std::string line;
				std::ifstream file(input_file_name);
				if (!file.is_open())
