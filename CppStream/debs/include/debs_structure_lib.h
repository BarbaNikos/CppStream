...
typedef struct compact_ride_str {
  ...
  compact_ride_str(const std::string& line) { parse_string(line); }
  ~compact_ride_str() {}
  ...
  std::string to_string() const {
    ...
    return med + "," + std::to_string(trip_distance) + "," +
           std::to_string(dropoff_datetime) + "," +
           std::to_string(pickup_cell.first) + "," +
           std::to_string(pickup_cell.second) + "," +
           std::to_string(dropoff_cell.first) + "," +
           std::to_string(dropoff_cell.second) + "," +
  ...
  void parse_string(const std::string& ride_info) {
    ...
    try {
      ...
      std::cerr << "parse_string():: failed on trip-distance: " << tokens[1]
                << ", line: " << ride_info << "\n";
    }
    try {
      dropoff_datetime = std::stoll(tokens[2]);
    } catch (const std::exception&) {
      std::cerr << "parse_string():: failed on drop-off datetime: " << tokens[2]
                << ", line: " << ride_info << "\n";
    }
    ...
    try {
      ...
      std::cerr << "parse_string():: failed on fare-amount: " << tokens[6]
                << ", line: " << ride_info << "\n";
    }
    try {
      tip_amount = std::stof(tokens[7]);
    } catch (const std::exception&) {
      std::cerr << "parse_string():: failed on tip-amount: " << tokens[7]
                << ", line: " << ride_info << "\n";
    }
  }
  char medallion[32];
...
typedef struct frequent_route_str {
  ...
  frequent_route_str(const std::string route, const uint64_t count)
...
typedef struct most_profitable_cell_str {
  ...
  most_profitable_cell_str(std::string cell, const double profit)
      : cell(cell), profit(profit) {}
