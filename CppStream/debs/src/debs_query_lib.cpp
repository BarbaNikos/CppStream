...
void Experiment::DebsChallenge::FrequentRouteWorker::update(
    const Experiment::DebsChallenge::CompactRide& ride) {
  std::stringstream str_stream;
  str_stream << (unsigned short)ride.pickup_cell.first << "."
             << (unsigned short)ride.pickup_cell.second << "-"
             << (unsigned short)ride.dropoff_cell.first << "."
             << (unsigned short)ride.dropoff_cell.second;
  std::unordered_map<std::string, uint64_t>::iterator it =
      result.find(str_stream.str());
  if (it != result.end()) {
    it->second += 1;
  } else {
...
void Experiment::DebsChallenge::FrequentRouteWorker::finalize(
    bool write,
    std::vector<Experiment::DebsChallenge::frequent_route>* partial_result) {
  if (write) {
    for (std::unordered_map<std::string, uint64_t>::const_iterator it =
             result.cbegin();
         it != result.cend(); ++it) {
...
void Experiment::DebsChallenge::FrequentRouteAggregator::final_aggregate(
    const std::vector<Experiment::DebsChallenge::frequent_route>&
        partial_results,
    std::unordered_map<std::string, uint64_t>& sorted_full_aggregates) {
  for (auto cit = partial_results.cbegin(); cit != partial_results.cend();
       ++cit) {
...
void Experiment::DebsChallenge::FrequentRouteAggregator::order_final_result(
    const std::vector<Experiment::DebsChallenge::frequent_route>&
        full_aggregates,
    std::vector<std::pair<unsigned long, std::string>>& result) {
  std::map<unsigned long, std::vector<std::string>> final_result;
  for (std::vector<frequent_route>::const_iterator cit =
           full_aggregates.cbegin();
       cit != full_aggregates.cend(); ++cit) {
...
void Experiment::DebsChallenge::FrequentRouteAggregator::order_final_result(
    const std::unordered_map<std::string, uint64_t>& full_aggregates,
    std::vector<std::pair<unsigned long, std::string>>& result) {
  // START: sort full result (all partitioners required)
  std::map<unsigned long, std::vector<std::string>> final_result;
  for (auto cit = full_aggregates.cbegin(); cit != full_aggregates.cend();
       ++cit) {
...
void Experiment::DebsChallenge::FrequentRouteAggregator::write_output_to_file(
    const std::vector<std::pair<unsigned long, std::string>>& result,
...
void Experiment::DebsChallenge::FrequentRoutePartition::
    frequent_route_simulation(
        const std::vector<Experiment::DebsChallenge::CompactRide>& lines,
  ...
  const std::string sh_file_name =
      "sh_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string fld_file_name =
      "fld_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string pkg_file_name =
      "pk_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string ca_naive_file_name =
      "cn_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string ca_aff_naive_file_name =
      "an_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string ca_hll_file_name =
      "chll_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string ca_aff_hll_file_name =
      "ahll_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string la_naive_file_name =
      "ln_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  const std::string la_hll_file_name =
      "lhll_debs_q1_" + std::to_string(tasks.size()) + ".csv";
  std::cout << "DEBS Q1***\n";
  std::stringstream info_stream;
  info_stream << "partitioner,task-number,min-exec-msec,max-exec-msec,avg-exec-"
                 "msec,avg-aggr-msec,io-msec,order-msec,imb,key-imb\n";
  std::cout << info_stream.str();
  frequent_route_partitioner_simulation(true, lines, tasks, rrg, "sh",
                                        sh_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, fld, "fld",
                                        fld_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, pkg, "pk",
                                        pkg_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, ca_naive, "cn",
                                        ca_naive_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, ca_aff_naive, "an",
                                        ca_aff_naive_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, ca_hll, "chll",
                                        ca_hll_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, ca_aff_hll, "ahll",
                                        ca_aff_hll_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, la_naive, "ln",
                                        la_naive_file_name);
  frequent_route_partitioner_simulation(true, lines, tasks, la_hll, "lhll",
                                        la_hll_file_name);
  tasks.clear();
...
void Experiment::DebsChallenge::FrequentRoutePartition::partition(
    std::unique_ptr<Partitioner>& partitioner,
    const std::vector<CompactRide>& rides,
    std::vector<std::vector<CompactRide>>& worker_input_buffer,
    size_t task_number, float* imbalance, float* key_imbalance) {
  DebsFrequentRideKeyExtractor key_extractor;
  ImbalanceScoreAggr<CompactRide, std::string> imbalance_aggregator(
      task_number, key_extractor);
  ...
  for (auto it = worker_input_buffer.begin(); it != worker_input_buffer.end();
       ++it) {
    it->shrink_to_fit();
  }
...
void Experiment::DebsChallenge::FrequentRoutePartition::worker_thread_operate(
    bool write, std::vector<Experiment::DebsChallenge::CompactRide>* input,
...
void Experiment::DebsChallenge::FrequentRoutePartition::
    aggregation_thread_operate(
        bool write, std::string partitioner_name,
        std::vector<Experiment::DebsChallenge::frequent_route>* input_buffer,
        std::vector<std::pair<unsigned long, std::string>>* result,
        double* total_duration, std::string worker_output_file_name,
        double* io_duration, double* order_duration) {
...
std::vector<double> Experiment::DebsChallenge::FrequentRoutePartition::
    frequent_route_partitioner_simulation(
        const bool write, const std::vector<CompactRide>& rides,
  ...
  std::thread** threads;
  double aggregate_durations[5], aggr_duration, write_output_duration,
      order_duration;
  std::vector<double> exec_durations(tasks.size(), double(0)), aggr_durations,
      partition_duration_vector;
  std::vector<frequent_route> partial_result;
  std::vector<std::vector<CompactRide>> worker_input_buffer(
      tasks.size(), std::vector<CompactRide>());
  std::vector<std::pair<unsigned long, std::string>> result;
  // partition tuples
  partition(partitioner, rides, worker_input_buffer, tasks.size(), &imbalance,
            &key_imbalance);
  ...
  for (size_t i = 0; i < tasks.size(); ++i) {
    ...
    for (size_t run = 0; run < 5; ++run) {
      threads[run] = new std::thread(
          Experiment::DebsChallenge::FrequentRoutePartition::
              worker_thread_operate,
    ...
    auto max_it =
        std::max_element(task_durations.begin(), task_durations.end());
    task_durations.erase(max_it);
    auto min_it =
        std::min_element(task_durations.begin(), task_durations.end());
    task_durations.erase(min_it);
    double average_exec_duration =
        std::accumulate(task_durations.begin(), task_durations.end(), 0.0) /
        task_durations.size();
  ...
  for (size_t run = 0; run < 5; ++run) {
    threads[run] =
        new std::thread(Experiment::DebsChallenge::FrequentRoutePartition::
                            aggregation_thread_operate,
                        (run == 0), partitioner_name, &partial_result, &result,
                        &aggregate_durations[run], worker_output_file_name,
                        &write_output_duration, &order_duration);
  }
  for (size_t run = 0; run < 5; ++run) {
    threads[run]->join();
    delete threads[run];
  ...
  if (write) {
    ...
    result_stream
        ...
        << order_duration << "," << imbalance << "," << key_imbalance << "\n";
    std::cout << result_stream.str();
  }
  std::vector<double> partition_results;
  partition_results.push_back(
      *std::min_element(exec_durations.begin(), exec_durations.end()));
  partition_results.push_back(
      *std::max_element(exec_durations.begin(), exec_durations.end()));
  partition_results.push_back(
...
void Experiment::DebsChallenge::FrequentRoutePartition::
    frequent_route_window_simulation(const std::string& file,
                                     const size_t task_number) {
  CardinalityAwarePolicy ca_policy;
  LoadAwarePolicy la_policy;
  std::vector<uint16_t> tasks(task_number, 0);
  ...
  std::stringstream info_stream;
  info_stream << "partitioner,task-num,min-s1-msec,max-s1-msec,75ile-max-s1-"
                 "msec,99ile-max-s1-msec,avg-s1-msec,"
              << "avg-s1-aggr-msec,99ile-s1-aggre,avg-order-msec,avg-imb,avg-"
                 "key-imb,mean-window\n";
  std::cout << info_stream.str();
  // frequent_route_partitioner_window_simulation(file, tasks, rrg,
  // StreamPartitionLib::Name::Util::shuffle_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, fld,
  // StreamPartitionLib::Name::Util::field_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, pkg,
  // StreamPartitionLib::Name::Util::partial_key_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, ca_naive,
  // StreamPartitionLib::Name::Util::cardinality_naive_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, ca_aff_naive,
  // StreamPartitionLib::Name::Util::affinity_naive_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, ca_hll,
  // StreamPartitionLib::Name::Util::cardinality_hip_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, ca_aff_hll,
  // StreamPartitionLib::Name::Util::affinity_hip_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, la_naive,
  // StreamPartitionLib::Name::Util::load_naive_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, la_hll,
  // StreamPartitionLib::Name::Util::load_hip_partitioner());
  // frequent_route_partitioner_window_simulation(file, tasks, man,
  // StreamPartitionLib::Name::Util::multi_affinity_naive_partitioner(5));
  // frequent_route_partitioner_window_simulation(file, tasks, mpk,
  // StreamPartitionLib::Name::Util::multi_partial_key_partitioner(5));
  frequent_route_partitioner_window_simulation(
      file, tasks, can,
...
void Experiment::DebsChallenge::FrequentRoutePartition::
    frequent_route_partitioner_window_simulation(
        const std::string& rides, const std::vector<uint16_t>& tasks,
        std::unique_ptr<Partitioner>& partitioner,
        const std::string& partitioner_name) {
  std::vector<double> results, min_parallel_durations, max_parallel_durations,
      mean_parallel_durations;
  std::vector<double> aggregate_durations, io_durations, order_durations,
      imbalances, key_imbalances, window_sizes;
  ...
  while (std::getline(infile, line)) {
    ...
    if (sec_diff >= debs_window_size_in_sec) {
      ...
      partitioner->init();
      // process window
      auto window_results = frequent_route_partitioner_simulation(
          false, window_rides, tasks, partitioner, partitioner_name, "result");
  ...
  double seventyfive_ile_max_parallel_duration =
      StreamPartitionLib::Stats::Util::get_percentile(max_parallel_durations,
                                                      0.75);
  double ninety_ile_max_parallel_duration =
      StreamPartitionLib::Stats::Util::get_percentile(max_parallel_durations,
                                                      0.9);
  ...
  double mean_aggr_duration =
      StreamPartitionLib::Stats::Util::get_mean(aggregate_durations);
  double mean_aggr_99ile_duration =
      StreamPartitionLib::Stats::Util::get_percentile(aggregate_durations,
                                                      0.99);
  ...
  double mean_key_imbalance =
      StreamPartitionLib::Stats::Util::get_mean(key_imbalances);
  double mean_window_size =
      StreamPartitionLib::Stats::Util::get_mean(window_sizes);
  std::stringstream s_stream;
  s_stream << partitioner_name << "," << tasks.size() << ","
           << mean_min_parallel_duration << "," << mean_max_parallel_duration
           << "," << seventyfive_ile_max_parallel_duration << ","
           << ninety_ile_max_parallel_duration << ","
           << mean_mean_parallel_duration << "," << mean_aggr_duration << ","
           << mean_aggr_99ile_duration << "," << mean_order_duration << ","
           << mean_imbalance << "," << mean_key_imbalance << ","
           << mean_window_size << "\n";
  std::cout << s_stream.str();
}

void Experiment::DebsChallenge::ProfitableAreaWorker::update(
    const DebsChallenge::CompactRide& ride) {
  float total = ride.fare_amount + ride.tip_amount;
  auto pickup_cell = std::to_string(ride.pickup_cell.first) + "." +
                     std::to_string(ride.pickup_cell.second);
  auto dropoff_cell = std::to_string(ride.dropoff_cell.first) + "." +
                      std::to_string(ride.dropoff_cell.second);
  auto medal = std::string(ride.medallion,
                           sizeof(ride.medallion) / sizeof(ride.medallion[0]));
  // update fare table
...
void Experiment::DebsChallenge::ProfitableAreaWorker::first_round_gather(
    std::vector<std::pair<std::string, std::vector<float>>>& fare_table,
  ...
  for (auto it = this->dropoff_table.cbegin(); it != this->dropoff_table.cend();
       ++it) {
...
void Experiment::DebsChallenge::ProfitableAreaWorker::second_round_update(
    const std::string& pickup_cell, const std::vector<float>& fare_list) {
  // calculate median
  float median_fare = 0.0;
  std::vector<float> fare_list_copy(fare_list);
  if (fare_list.size() > 0) {
    if (fare_list.size() % 2 != 0) {
      std::nth_element(fare_list_copy.begin(),
                       fare_list_copy.begin() + fare_list_copy.size() / 2,
                       fare_list_copy.end());
    ...
    if (it != pickup_cell_median_fare.end()) {
      // this is probably a mistake since each pickup-cell has a single list of
      // fare lists
      // it->second = median_fare;
      std::cout << "second_round_update() identified duplicate pickup-cell "
                   "record. the cell-id: "
                << pickup_cell
                << " was encountered twice. current median value found: "
                << it->second << ", new value: " << median_fare << ".\n";
    } else {
...
void Experiment::DebsChallenge::ProfitableAreaWorker::second_round_update(
    const std::string& medallion, const std::string& dropoff_cell,
...
void Experiment::DebsChallenge::ProfitableAreaWorker::
    partial_second_step_finalize(
        std::vector<std::pair<std::string, std::pair<float, int>>>&
            partial_result_table) const {
  std::unordered_map<std::string, std::pair<float, int>> buffer;
  for (auto dropoff_cell_it = dropoff_cell_empty_taxi_count.cbegin();
       dropoff_cell_it != dropoff_cell_empty_taxi_count.cend();
       dropoff_cell_it++) {
    buffer[dropoff_cell_it->first] =
        std::make_pair(0.0, dropoff_cell_it->second);
  }
  for (auto fare_it = pickup_cell_median_fare.cbegin();
       fare_it != pickup_cell_median_fare.cend(); ++fare_it) {
  ...
  for (auto buffer_it = buffer.cbegin(); buffer_it != buffer.cend();
       ++buffer_it) {
    partial_result_table.push_back(*buffer_it);
  }
}

void Experiment::DebsChallenge::ProfitableAreaWorker::partial_finalize(
    std::unordered_map<std::string, std::pair<float, int>>& cell_profit_buffer)
    const {
  for (auto dropoff_cell_it = dropoff_cell_empty_taxi_count.cbegin();
       dropoff_cell_it != dropoff_cell_empty_taxi_count.cend();
       ++dropoff_cell_it) {
    auto cell_profit_buffer_it =
        cell_profit_buffer.find(dropoff_cell_it->first);
    if (cell_profit_buffer_it != cell_profit_buffer.end()) {
      cell_profit_buffer_it->second.second += dropoff_cell_it->second;
    } else {
      cell_profit_buffer[dropoff_cell_it->first] =
          std::make_pair(0.0, dropoff_cell_it->second);
    }
  }
  for (auto fare_it = pickup_cell_median_fare.cbegin();
...
void Experiment::DebsChallenge::ProfitableAreaWorker::finalize(
    std::unordered_map<std::string, float>& cell_profit_buffer) {
  for (auto dropoff_cell_it = dropoff_cell_empty_taxi_count.cbegin();
       dropoff_cell_it != dropoff_cell_empty_taxi_count.cend();
       ++dropoff_cell_it) {
...
void Experiment::DebsChallenge::ProfitableAreaAggregator::
    step_one_materialize_aggregation(
        const std::vector<std::pair<std::string, std::vector<float>>>&
            fare_table,
        std::vector<std::pair<std::string, std::vector<float>>>&
            final_fare_table,
        const std::vector<std::pair<
            std::string, std::pair<std::string, std::time_t>>>& dropoff_table,
        std::vector<
            std::pair<std::string, std::pair<std::string, std::time_t>>>&
            final_dropoff_table) {
  std::unordered_map<std::string, std::vector<float>> complete_fare_table;
  std::unordered_map<std::string, std::pair<std::string, std::time_t>>
      complete_dropoff_table;
  for (auto it = fare_table.cbegin(); it != fare_table.cend(); ++it) {
    ...
    if (c_ft != complete_fare_table.end()) {
      c_ft->second.insert(c_ft->second.end(), it->second.begin(),
                          it->second.end());
    } else {
      complete_fare_table[it->first] = it->second;
    }
  }
  for (auto it = complete_fare_table.cbegin(); it != complete_fare_table.cend();
       ++it) {
  ...
  for (auto it = complete_dropoff_table.cbegin();
       it != complete_dropoff_table.cend(); ++it) {
    final_dropoff_table.push_back(*it);
  }
}

void Experiment::DebsChallenge::ProfitableAreaAggregator::order_final_result(
    const std::unordered_map<std::string, float>& cell_profit_buffer,
    std::vector<std::pair<float, std::string>>& final_result) {
  std::map<float, std::vector<std::string>> ordered_result;
  for (std::unordered_map<std::string, float>::const_iterator cit =
           cell_profit_buffer.cbegin();
       cit != cell_profit_buffer.cend(); ++cit) {
  ...
  for (std::map<float, std::vector<std::string>>::const_reverse_iterator c =
           ordered_result.crbegin();
       c != ordered_result.crend(); ++c) {
    for (std::vector<std::string>::const_iterator i = c->second.cbegin();
         i != c->second.cend(); ++i) {
...
void Experiment::DebsChallenge::ProfitableAreaAggregator::order_final_result(
    const std::unordered_map<std::string, std::pair<float, int>>&
        profit_final_buffer,
    std::vector<std::pair<float, std::string>>& final_result) {
  std::map<float, std::vector<std::string>> ordered_result;
  for (auto cit = profit_final_buffer.cbegin();
       cit != profit_final_buffer.cend(); ++cit) {
...
void Experiment::DebsChallenge::ProfitableAreaAggregator::
    calculate_final_result(
        const std::unordered_map<std::string, std::pair<float, int>>&
            cell_profit_buffer,
        std::unordered_map<std::string, std::pair<float, int>>&
            profit_final_buffer) {
  for (auto cit = cell_profit_buffer.cbegin(); cit != cell_profit_buffer.cend();
       ++cit) {
...
void Experiment::DebsChallenge::ProfitableAreaAggregator::output_result_to_file(
    const std::vector<std::pair<float, std::string>>& final_result,
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    most_profitable_cell_simulation(
        const std::vector<Experiment::DebsChallenge::CompactRide>& lines,
        const size_t task_number) {
  CardinalityAwarePolicy ca_policy;
  LoadAwarePolicy la_policy;
  ...
  std::string sh_file_name =
      "sh_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string fld_file_name =
      "fld_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string pkg_file_name =
      "pk_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string ca_naive_file_name =
      "cn_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string ca_aff_naive_file_name =
      "an_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string ca_hll_file_name =
      "chll_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string ca_aff_hll_file_name =
      "ahll_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string la_naive_file_name =
      "ln_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::string la_hll_file_name =
      "lhll_debs_q2_" + std::to_string(tasks.size()) + ".csv";
  std::cout << "DEBS Q2 ***\n";
  std::stringstream info_stream;
  info_stream << "partitioner,task-num,min-s1-msec,max-s1-msec,avg-s1-msec,avg-"
                 "s1-aggr-msec,min-s2-msec,max-s2-msec,avg-s2-msec,avg-aggr-"
                 "msec,io-msec,order-msec,avg-part-cell-msec,"
              << "med-imb,med-key-imb,dropoff-imb,dropoff-key-imb,fare-imb,"
                 "fare-key-imb\n";
  std::cout << info_stream.str();
  most_profitable_partitioner_simulation(true, lines, tasks, rrg, "sh",
                                         sh_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, fld, "fld",
                                         fld_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, pkg, "pk",
                                         pkg_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, ca_naive, "cn",
                                         ca_naive_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, ca_aff_naive, "an",
                                         ca_aff_naive_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, ca_hll, "chll",
                                         ca_hll_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, ca_aff_hll, "ahll",
                                         ca_aff_hll_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, la_naive, "ln",
                                         la_naive_file_name);
  most_profitable_partitioner_simulation(true, lines, tasks, la_hll, "lhll",
                                         la_hll_file_name);
...
void Experiment::DebsChallenge::ProfitableAreaPartition::partition_medallion(
    std::unique_ptr<Partitioner>& partitioner,
    const std::vector<Experiment::DebsChallenge::CompactRide>& buffer,
    size_t task_number, float* imbalance, float* key_imbalance,
    std::vector<std::vector<CompactRide>>* worker_buffer) {
  std::chrono::duration<double, std::milli> total_duration;
  DebsFrequentRideKeyExtractor key_extractor;
  ImbalanceScoreAggr<Experiment::DebsChallenge::CompactRide, std::string>
      imbalance_aggregator(task_number, key_extractor);
  std::chrono::system_clock::time_point part_start =
      std::chrono::system_clock::now();
  ...
  std::chrono::system_clock::time_point part_end =
      std::chrono::system_clock::now();
...
void Experiment::DebsChallenge::ProfitableAreaPartition::partition_step_two(
    std::unique_ptr<Partitioner>& partitioner,
    std::vector<std::pair<std::string, std::vector<float>>>* fares,
    std::vector<std::pair<std::string, std::pair<std::string, std::time_t>>>*
        dropoffs,
    size_t task_number, float* fare_imbalance, float* fare_key_imbalance,
    float* dropoff_imbalance, float* dropoff_key_imbalance,
  ...
  ImbalanceScoreAggr<std::pair<std::string, std::vector<float>>, std::string>
      complete_fare_imb_aggregator(task_number, complete_fare_key_extractor);
  ImbalanceScoreAggr<
      std::pair<std::string, std::pair<std::string, std::time_t>>, std::string>
      dropoff_cell_imb_aggregator(task_number, dropoff_cell_key_extractor);
  ...
  for (auto it = dropoffs->cbegin(); it != dropoffs->cend(); ++it) {
    ...
    if (task < (*dropoffs_sub_table).size()) {
      (*dropoffs_sub_table)[task].push_back(
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    thread_execution_step_one(
        bool write,
        const std::vector<Experiment::DebsChallenge::CompactRide>* input_buffer,
        std::vector<std::pair<std::string, std::vector<float>>>* fare_table,
        std::vector<std::pair<std::string, std::pair<std::string, time_t>>>*
            dropoff_table,
        double* total_duration) {
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    thread_execution_aggregation_step_one(
        bool write, const std::string partitioner_name,
        const std::vector<std::pair<std::string, std::vector<float>>>*
            fare_table,
        const std::vector<std::pair<
            std::string, std::pair<std::string, std::time_t>>>* dropoff_table,
        std::vector<std::pair<std::string, std::vector<float>>>* fare_table_out,
        std::vector<
            std::pair<std::string, std::pair<std::string, std::time_t>>>*
            dropoff_table_out,
        double* total_duration) {
  ...
  if (write) {
    std::chrono::system_clock::time_point start =
        std::chrono::system_clock::now();
    if (StreamPartitionLib::Name::Util::single_choice_partitioner(
            partitioner_name) == true) {
      // aggregator.step_one_aggregation(*fare_table, *fare_table_out,
      // *dropoff_table, *dropoff_table_out);
      // fld and Aff can not just transfer tuples in this because the
      // partitioning on step one takes place
      // on Medallion. However, in this aggregation the keys are the pickup and
      // dropoff cells. Therefore, this
      // aggregation has to take place
      aggregator.step_one_materialize_aggregation(
          *fare_table, *fare_table_out, *dropoff_table, *dropoff_table_out);
    } else {
      aggregator.step_one_materialize_aggregation(
          *fare_table, *fare_table_out, *dropoff_table, *dropoff_table_out);
    }
    ...
    if (partitioner_name.compare("fld") == 0 ||
        partitioner_name.compare("an") == 0 ||
        partitioner_name.compare("ahll") == 0) {
      aggregator.step_one_materialize_aggregation(
          *fare_table, fare_table_aux, *dropoff_table, dropoff_table_aux);
    } else {
      aggregator.step_one_materialize_aggregation(
          *fare_table, fare_table_aux, *dropoff_table, dropoff_table_aux);
    }
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    thread_final_aggregation(
        bool write, const std::string partitioner_name,
        std::unordered_map<std::string, std::pair<float, int>>*
            partial_input_buffer,
        const std::unordered_map<std::string, float>* full_input_buffer,
        std::vector<std::pair<float, std::string>>* final_result,
        const std::string worker_output_file_name, double* total_duration,
        double* io_duration, double* order_duration) {
  ...
  if (write) {
    if (StreamPartitionLib::Name::Util::single_choice_partitioner(
            partitioner_name) == false) {
      start = std::chrono::system_clock::now();
      aggregator.calculate_final_result(*partial_input_buffer,
                                        profit_final_buffer);
      end = std::chrono::system_clock::now();
      aggr_time = end - start;
      start = std::chrono::system_clock::now();
    ...
    std::chrono::duration<double, std::milli> output_to_file_duration =
        (end - start);
...
std::vector<double> Experiment::DebsChallenge::ProfitableAreaPartition::
    most_profitable_partitioner_simulation(
        bool write, const std::vector<CompactRide>& rides,
        const std::vector<uint16_t>& tasks,
        std::unique_ptr<Partitioner>& partitioner,
        const std::string& partitioner_name,
        const std::string& worker_output_file_name) {
  std::thread** threads;
  float med_imbalance, med_key_imbalance, dropoff_cell_imbalance,
      dropoff_cell_key_imbalance, complete_fare_imbalance,
      complete_fare_key_imbalance;
  double step_one_aggr_durations[5], step_one_durations[5], final_aggr_duration,
      output_to_file_duration, order_duration;
  std::vector<double> first_round_exec_duration(tasks.size(), double(0)),
      first_aggr_durations, second_round_exec_duration(tasks.size(), double(0));
  // buffers
  std::vector<std::vector<CompactRide>> worker_input_buffer(
      tasks.size(), std::vector<CompactRide>());
  std::vector<std::pair<std::string, std::vector<float>>> fare_table,
      final_fare_table;
  std::vector<std::pair<std::string, std::pair<std::string, std::time_t>>>
      dropoff_table, final_dropoff_table;
  std::vector<std::vector<std::pair<std::string, std::vector<float>>>>
  complete_fare_sub_table(
      tasks.size(), std::vector<std::pair<std::string, std::vector<float>>>());
  std::vector<
      std::vector<std::pair<std::string, std::pair<std::string, std::time_t>>>>
      dropoff_cell_sub_table(
          tasks.size(),
          std::vector<
              std::pair<std::string, std::pair<std::string, std::time_t>>>());
  std::unordered_map<std::string, std::pair<float, int>>
      cell_partial_result_buffer;
  ...
  // S1 (parallel): for each pickup-cell gather fares, for each medallion keep
  // track of the latest dropoff-cell
  for (size_t task = 0; task < tasks.size(); ++task) {
    std::vector<double> durations;
    threads = new std::thread*[5];
    for (size_t run = 0; run < 5; ++run) {
      threads[run] =
          new std::thread(Experiment::DebsChallenge::ProfitableAreaPartition::
                              thread_execution_step_one,
                          (run == 0), &worker_input_buffer[task], &fare_table,
                          &dropoff_table, &step_one_durations[run]);
    }
    for (size_t run = 0; run < 5; ++run) {
      threads[run]->join();
      delete threads[run];
    ...
    first_round_exec_duration[task] =
        std::accumulate(durations.begin(), durations.end(), 0.0) /
        durations.size();
    worker_input_buffer[task].clear();
    durations.clear();
  }
  // S1 aggr (serial): fare_table and dropoff_table reduction (duplicate
  // elimination)
  threads = new std::thread*[5];
  for (size_t run = 0; run < 5; ++run) {
    threads[run] = new std::thread(
        Experiment::DebsChallenge::ProfitableAreaPartition::
            thread_execution_aggregation_step_one,
        (run == 0), partitioner_name, &fare_table, &dropoff_table,
        &final_fare_table, &final_dropoff_table, &step_one_aggr_durations[run]);
  }
  for (size_t run = 0; run < 5; ++run) {
    threads[run]->join();
    delete threads[run];
  ...
  fare_table.clear();
  dropoff_table.clear();
  // Partition Data: table fare_table on pickup_cell, table dropoff_table on
  // dropoff_cell
  partition_step_two(partitioner, &final_fare_table, &final_dropoff_table,
                     tasks.size(), &complete_fare_imbalance,
                     &complete_fare_key_imbalance, &dropoff_cell_imbalance,
                     &dropoff_cell_key_imbalance, &complete_fare_sub_table,
                     &dropoff_cell_sub_table);
  final_fare_table.clear();
  final_dropoff_table.clear();
  // S2 (parallel): Calculate median for each pickup_cell, and number of empty
  // taxis per dropoff_cell - at this point there is a unique record
  //					for each pickup_cell and for each
  //medallion
  //(but
  // medallions might end up in a different place)
  for (size_t i = 0; i < tasks.size(); ++i) {
    std::vector<double> durations;
    for (size_t run = 0; run < 5; ++run) {
      ...
      std::unordered_map<std::string, std::pair<float, int>>
          cell_partial_result_buffer_copy(cell_partial_result_buffer);
      std::unordered_map<std::string, float> cell_full_result_buffer_copy(
          cell_full_result_buffer);
      std::chrono::system_clock::time_point start =
          std::chrono::system_clock::now();
      for (auto it = complete_fare_sub_table[i].cbegin();
           it != complete_fare_sub_table[i].cend(); ++it) {
        worker.second_round_update(it->first, it->second);
      }
      for (auto it = dropoff_cell_sub_table[i].cbegin();
           it != dropoff_cell_sub_table[i].cend(); ++it) {
        worker.second_round_update(it->first, it->second.first,
                                   it->second.second);
      }
      if (StreamPartitionLib::Name::Util::single_choice_partitioner(
              partitioner_name) == false) {
  ...
  // Final aggregation (serial): Gather all records and materialize result. Sort
  // and produce Top-10.
  std::vector<std::pair<float, std::string>> final_result;
  ...
  for (size_t aggr_run = 0; aggr_run < 5; ++aggr_run) {
    threads[aggr_run] = new std::thread(
        thread_final_aggregation, (aggr_run == 0), partitioner_name,
        &cell_partial_result_buffer, &cell_full_result_buffer, &final_result,
        worker_output_file_name, &final_aggregate_durations[aggr_run],
        &output_to_file_duration, &order_duration);
  }
  for (size_t aggr_run = 0; aggr_run < 5; ++aggr_run) {
  ...
  final_aggr_duration =
      ...
      aggr_durations.size();
  aggr_durations.clear();
  min_it = std::min_element(first_aggr_durations.begin(),
                            first_aggr_durations.end());
  first_aggr_durations.erase(min_it);
  max_it = std::max_element(first_aggr_durations.begin(),
                            first_aggr_durations.end());
  first_aggr_durations.erase(max_it);
  if (write) {
    std::stringstream result_stream;
    result_stream << partitioner_name << "," << tasks.size() << ","
                  << *std::min_element(first_round_exec_duration.begin(),
                                       first_round_exec_duration.end())
                  << ","
                  << *std::max_element(first_round_exec_duration.begin(),
                                       first_round_exec_duration.end())
                  << ","
                  << (std::accumulate(first_round_exec_duration.begin(),
                                      first_round_exec_duration.end(), 0.0) /
                      first_round_exec_duration.size())
                  << "," << (std::accumulate(first_aggr_durations.begin(),
                                             first_aggr_durations.end(), 0.0) /
                             first_aggr_durations.size())
                  << ","
                  << *std::min_element(second_round_exec_duration.begin(),
                                       second_round_exec_duration.end())
                  << ","
                  << *std::max_element(second_round_exec_duration.begin(),
                                       second_round_exec_duration.end())
                  << ","
                  << (std::accumulate(second_round_exec_duration.begin(),
                                      second_round_exec_duration.end(), 0.0) /
                      second_round_exec_duration.size())
                  << "," << final_aggr_duration << ","
                  << output_to_file_duration << "," << order_duration << ",N/A,"
                  << med_imbalance << "," << med_key_imbalance << ","
                  << dropoff_cell_imbalance << "," << dropoff_cell_key_imbalance
                  << "," << complete_fare_imbalance << ","
                  << complete_fare_key_imbalance << "\n";
    std::cout << result_stream.str();
  }
  ...
  results.push_back(*std::min_element(first_round_exec_duration.begin(),
                                      first_round_exec_duration.end()));
  results.push_back(*std::max_element(first_round_exec_duration.begin(),
                                      first_round_exec_duration.end()));
  results.push_back((std::accumulate(first_round_exec_duration.begin(),
                                     first_round_exec_duration.end(), 0.0) /
                     first_round_exec_duration.size()));
  results.push_back((std::accumulate(first_aggr_durations.begin(),
                                     first_aggr_durations.end(), 0.0) /
                     first_aggr_durations.size()));
  results.push_back(*std::min_element(second_round_exec_duration.begin(),
                                      second_round_exec_duration.end()));
  results.push_back(*std::max_element(second_round_exec_duration.begin(),
                                      second_round_exec_duration.end()));
  results.push_back((std::accumulate(second_round_exec_duration.begin(),
                                     second_round_exec_duration.end(), 0.0) /
                     second_round_exec_duration.size()));
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    profitable_route_window_simulation(const std::string& file,
                                       const size_t task_number) {
  CardinalityAwarePolicy ca_policy;
  LoadAwarePolicy la_policy;
  std::vector<uint16_t> tasks(task_number, 0);
  ...
  std::stringstream info_stream;
  info_stream << "name,task-num,min-s1,max-s1,max-99ile-s1,mean-s1,s1-aggr,s1-"
                 "aggr-99ile,min-s2,max-s2,max-99ile-s2,"
              << "mean-s2,s2-aggr,s2-aggr-99ile,med-imb,med-k-imb,cell-imb,"
                 "cell-k-imb,fare-imb,fare-k-imb,mean-window\n";
  std::cout << info_stream.str();
  profitable_route_partitioner_window_simulation(
      file, tasks, rrg, StreamPartitionLib::Name::Util::shuffle_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, fld, StreamPartitionLib::Name::Util::field_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, pkg,
      StreamPartitionLib::Name::Util::partial_key_partitioner());
  profitable_route_partitioner_window_simulation(
      ...
      StreamPartitionLib::Name::Util::cardinality_naive_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, ca_aff_naive,
      StreamPartitionLib::Name::Util::affinity_naive_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, ca_hll,
      StreamPartitionLib::Name::Util::cardinality_hip_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, ca_aff_hll,
      StreamPartitionLib::Name::Util::affinity_hip_partitioner());
  profitable_route_partitioner_window_simulation(
      file, tasks, la_naive,
      StreamPartitionLib::Name::Util::load_naive_partitioner());
...
void Experiment::DebsChallenge::ProfitableAreaPartition::
    profitable_route_partitioner_window_simulation(
        ...
        std::unique_ptr<Partitioner>& partitioner,
        const std::string& partitioner_name) {
  std::vector<double> min_s1_parallel_durations, max_s1_parallel_durations,
      mean_s1_parallel_durations;
  std::vector<double> min_s2_parallel_durations, max_s2_parallel_durations,
      mean_s2_parallel_durations;
  std::vector<double> aggregate_s1_durations, final_aggr_durations,
      order_durations, med_imbalances, cell_imbalances, fare_imbalances,
      med_key_imbalances, cell_key_imbalances, fare_key_imbalances,
      window_sizes;
  ...
  while (std::getline(infile, line)) {
    ...
    if (sec_diff >= debs_window_size_in_sec) {
      ...
      partitioner->init();
      std::vector<double> window_results =
          most_profitable_partitioner_simulation(false, window_rides, tasks,
  ...
  if (window_rides.size() > 0) {
    // I am not sure about the following, but I think it is required
    partitioner->init();
    std::vector<double> window_results = most_profitable_partitioner_simulation(
        false, window_rides, tasks, partitioner, partitioner_name, "result");
    window_results.push_back(window_rides.size());
    window_results.shrink_to_fit();
    min_s1_parallel_durations.push_back(std::move(window_results[0]));
  ...
  s_stream
      ...
      << "," << StreamPartitionLib::Stats::Util::get_percentile(
                    max_s1_parallel_durations, 0.99)
      << ","
      ...
      << "," << StreamPartitionLib::Stats::Util::get_percentile(
                    max_s2_parallel_durations, 0.99)
      << ","
