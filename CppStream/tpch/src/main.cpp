...
int main(int argc, char** argv) {
  if (argc < 4) {
    std::cout
        << "usage: <customer_file.tbl> <lineitem_file.tbl> <orders_file.tbl>\n";
    exit(1);
  }
  ...
  */
  ...
  Experiment::Tpch::DataParser::parse_tpch_lineitem(lineitem_file,
                                                    lineitem_table);
  ...
  Experiment::Tpch::DataParser::parse_tpch_q3_customer(customer_file,
                                                       customer_table);
  Experiment::Tpch::DataParser::parse_tpch_order(orders_file, order_table);
  Experiment::Tpch::QueryThreePartition::query_three_simulation(
      customer_table, lineitem_table, order_table, 8);
  Experiment::Tpch::QueryThreePartition::query_three_simulation(
      customer_table, lineitem_table, order_table, 16);
  Experiment::Tpch::QueryThreePartition::query_three_simulation(
      customer_table, lineitem_table, order_table, 32);
