...
typedef struct query_one_result_str {
  std::string to_string() const {
    return std::to_string(return_flag) + "," + std::to_string(line_status) +
           ...
           std::to_string(sum_base_price) + "," +
           std::to_string(sum_disc_price) + "," + std::to_string(sum_charge) +
           "," + std::to_string(avg_qty / count_order) + "," +
           std::to_string(avg_price / count_order) + "," +
...
typedef struct query_three_result_str {
  ...
  query_three_result_str(uint32_t o_orderkey, uint32_t o_shippriority,
                         Tpch::date o_orderdate, float revenue) {
...
class QueryOneOfflineAggregator {
 public:
  ...
  static void order_result(
      ...
      std::map<std::string, query_one_result>& ordered_result);
  static void aggregate_final_result(
      const std::vector<query_one_result>& buffer,
      std::unordered_map<std::string, query_one_result>& result);
  static void write_output_result(
...
class QueryOnePartition {
 public:
  static void query_one_simulation(const std::vector<lineitem>& lines,
                                   const size_t task_num);
  static void lineitem_partition(
 ...
 public:
  ...
  void step_one_partial_finalize(
      ...
      std::unordered_map<uint32_t, query_three_step_one>&
          step_one_result_buffer) const;
  void step_two_init(const std::unordered_map<uint32_t, query_three_step_one>&
                         step_one_result);
  void step_two_update(const lineitem& line_item);
  void finalize(std::vector<std::pair<std::string, query_three_result>>&
                    result_buffer) const;
...
class QueryThreeOfflineAggregator {
 public:
  ...
  static void step_two_materialize(
      const std::vector<std::pair<std::string, query_three_result>>& buffer,
      std::unordered_map<std::string, query_three_result>& result);
  static void write_output_to_file(
      const std::vector<std::pair<std::string, query_three_result>>&
          final_result,
      const std::string& output_file);
};
...
class QueryThreePartition {
 public:
  static void query_three_simulation(
      const std::vector<Experiment::Tpch::q3_customer>& c_table,
      const std::vector<lineitem>& li_table, const std::vector<order>& o_table,
      const size_t task_num);
  static void customer_partition(
      std::unique_ptr<Partitioner>& partitioner,
  ...
  static void lineitem_partition(
      ...
      float* imbalance, float* key_imbalance);
  static void query_three_partitioner_simulation(
      const std::vector<q3_customer>& c_table,
      const std::vector<lineitem>& li_table, const std::vector<order>& o_table,
      const std::vector<uint16_t>& tasks,
      std::unique_ptr<Partitioner>& partitioner,
